...
<div id="jssor_1"
    ...
    <div data-u="loading" class="jssorl-009-spin"
        ...
        <img style="margin-top:-19px;position:relative;top:50%;width:38px;height:38px;"
             src="{{ asset('svg/loading/static-svg/spin.svg') }}"/>
    </div>
    <div data-u="slides"
        ...
        @foreach($banners as $index => $banner)
            <div>
                <div class="carousel-caption">
                    <h3 style="color: red; font-weight: 700;">{{ $banner->title }}<span>{{ $banner->discount }}</span>
                    </h3>
                    <p>{{ $banner->sub_title }}</p>

                    @switch($index+1)
                        @case(1)
                        <a class="hvr-outline-out button2 scroll" href="#noi-bat">Sản phẩm nổi bật</a>
                        @break
                        @case(2)
                        <a class="hvr-outline-out button2 scroll" href="#chinh">Sản phẩm chính</a>
                        @break
                    @endswitch
